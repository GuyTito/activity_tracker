...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  computed: {
    ...
    updated_today: function updated_today() {
      var today = this.$store.state.activities.filter(function (activity) {
        if (!activity.activity_updates.length) return false;
        var latest_update = activity.activity_updates.slice(-1)[0];
        return moment(latest_update.created_at).format("D-MMM-YYYY") == moment().format("D-MMM-YYYY");
      });
      return today;
    },
    created_today: function created_today() {
      var today = this.$store.state.activities.filter(function (activity) {
        return moment(activity.created_at).format("D-MMM-YYYY") == moment().format("D-MMM-YYYY");
      });
      return today;
    },
    all_activities: function all_activities() {
  ...
  return _c("div", { staticClass: "container mt-3" }, [
    _c(
      ...
      [
        ...
        _c("ActivityList", {
          attrs: {
            activities: _vm.updated_today,
            title: "Activities Updated Today"
          }
        }),
        _vm._v(" "),
        _c("hr"),
        ...
        _c("ActivityList", {
          attrs: {
            activities: _vm.created_today,
            title: "Activities Created Today"
          }
        }),
        _vm._v(" "),
        _c("hr"),
        _vm._v(" "),
        _c("ActivityList", {
          attrs: { activities: _vm.all_activities, title: "All Activities" }
        })
      ],
      1
    )
